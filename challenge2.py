...
from challenge1 import Challange1
...
class Challange2(Challange1):
    ...
    def __init__ (self, MAX_IMAGES=None):
        ...
        self.clone_servers=[]    
        self.clone_servers_build_status=[ 0 for i in self._cs_range() ]
...
""" % (PROGRAM_NAME, PROGRAM_NAME)
    ...
    def show_images(self):
        ...
        log ("Created cloud server images details:") 

    def show_clones(self):
        debug("show_clones start")

        log ("Created cloned cloud server details:") 
        self._show(self.clone_servers)

    def find_cloud(self, id):
        try:
            s=self.cs.servers.get(id)
            for i in self._cs_range():
                self.clone_servers.append(s)
    ...
    def delete_cloned_servers(self):
        ...
        self._delete_servers(self.clone_servers) 
    ...
    def run(self):
        ...
        # log("Building %d new cloud server(s) from images." % self.MAX_SERVERS)
        # self.build_servers_from_image()
        # self.set_current_build(self.clone_servers, self.clone_servers_build_status)
        # self.wait_for_build()
